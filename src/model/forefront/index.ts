...
export class Forefrontnew extends Chat {
  ...
  constructor(options?: ChatOptions) {
    ...
    this.pagePool = new BrowserPool<Account>(
      maxSize,
      initialAccounts.map((item) => item.id),
      this.init.bind(this)
    )
  }
  ...
  public async ask(req: Request): Promise<Response> {
    ...
    return new Promise((resolve) => {
      ...
      et.read(
        ({ event, data }: { event: unknown; data: string }) => {
          if (!data) {
            return
          }
          switch (event) {
  ...
  private async allowClipboard(browser: Browser, page: Page) {
    ...
    await context.overridePermissions('https://chat.forefront.ai', ['clipboard-read', 'clipboard-write'])
    await page.evaluate(() =>
