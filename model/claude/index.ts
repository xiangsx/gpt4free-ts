...
export class Claude extends Chat implements BrowserUser<Account> {
    ...
    async getOrgID(page: Page): Promise<string> {
        const req = await page.waitForRequest(req => req.url().indexOf('https://claude.ai/api/organizations') !== -1);
    ...
    async init(id: string, browser: Browser): Promise<[Page | undefined, Account]> {
        ...
        try {
            ...
            const element = await page.evaluateHandle(() => {
                const elements = Array.from(document.querySelectorAll('* > main > * > .contents > *'));
                return elements.find(element => element.textContent?.indexOf?.('Continue') !== -1);
            });
            //@ts-ignore
            await element.click();
            ...
            const continueLogin = await page.evaluateHandle(() => {
                const elements = Array.from(document.querySelectorAll('* > main > * > .contents > *'));
                return elements.find(element => element.textContent?.toLowerCase().indexOf('continue with login code') !== -1);
            });
            //@ts-ignore
            ...
            await page.waitForSelector('main > .flex > .max-w-sm > .mt-4 > button:nth-child(2)')
