...
class PoeAccountPool {
    ...
    constructor() {
        const pbList = (process.env.POEF_PB || '').split('|');
...
export class Poef extends Chat implements BrowserUser<Account> {
    ...
    constructor(options?: ChatOptions) {
        ...
        this.pagePool = new BrowserPool<Account>(+(process.env.POEF_POOL_SIZE || 0), this, false);
    }
    ...
    async init(id: string, browser: Browser): Promise<[Page | undefined, Account]> {
        ...
        try {
            ...
            await page.waitForSelector(Poef.InputSelector, {timeout: 30 * 1000, visible: true});
            await page.click(Poef.InputSelector);
            await page.type(Poef.InputSelector, `1`);
            if (!(await Poef.isVIP(page))) {
                account.invalid = true;
                this.accountPool.syncfile();
    ...
    public async askStream(req: ChatRequest, stream: EventStream) {
        ...
        try {
            ...
            const tt = setTimeout(async () => {
                ...
                await page.waitForSelector(Poef.ClearSelector);
